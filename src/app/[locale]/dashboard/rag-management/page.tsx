...
import { RAG_DOCUMENT_TYPES } from "@/constants/document-management";
...
export default function RagManagementPage() {
  ...
  const fetchDocuments = useCallback(async (page: number) => {
    setIsLoading(true);
    try {
      const response = await documentService.getRagDocuments({
        page,
        limit: PAGE_LIMIT,
      });
  ...
  const columns = useMemo(
    () =>
      getDocumentColumns({
        onView: (doc) => {
          router.push(`/en/dashboard/rag-management/${doc._id}`)
    ...
    <div className="h-full">
      ...
      <SectionWrapper>
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            <Typography variant="h1">Rag Management</Typography>
            <Typography variant="p" className="mt-1 text-gray-500">
              {totalCount} {totalCount > 1 ? "rag documents" : "rag document"} total
            </Typography>
          </div>
