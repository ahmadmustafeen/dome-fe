...
const CreateNewSiteCard = ({ onClick }: { onClick: () => void }) => {
  return (
    <div className='border-2 group cursor-pointer hover:bg-primary transition-all duration-500 hover:scale-[105%] rounded-xl py-10 w-[32.2%] bg-transparent flex justify-center items-center flex-col' onClick={onClick} >
      ...
      <p className='text-primary group-hover:text-white transition-all duration-500 text-2xl my-2'>Create Site</p>
      <p className='text-primary group-hover:text-white transition-all duration-500 text-xs'>Click here to create a new site</p>
    </div>
  )
}

export { CreateNewSiteCard }