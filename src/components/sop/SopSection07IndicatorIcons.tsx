...
type SopSection07IndicatorIconProps = {
  ariaHidden?: boolean;
  className?: string;
  indicatorId: string;
};

export const SopSection07IndicatorIcon = ({
  className,
  ariaHidden = true,
  indicatorId,
}: SopSection07IndicatorIconProps) => {
  if (indicatorId === MOP_IMPORTANT_INDICATOR_SELECT_EMPTY) {
    return null;
  }
  const resolved: LucideIcon | undefined =
    MOP_SECTION_07_IMPORTANT_INDICATOR_LUCIDE_BY_ID[
      indicatorId as MopImportantIndicatorId
    ];
  const IconComponent: LucideIcon =
    resolved !== undefined ? resolved : HelpCircle;
  return (
    <IconComponent
